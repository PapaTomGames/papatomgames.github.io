...
function getZombies(state) {
    return Array.from(state.objects.values()).filter(o => o.type === 'ZOMBIE');
}
function findObjectsOfType(state, type) {
...
async function testLevelSpawning() {
    ...
    for (let level = 1; level <= 10; level++) {
        ...
        assert(zombies.length >= config.minZombies && zombies.length <= config.maxZombies, `Level ${level}: ${zombies.length} zombies (expected ${config.minZombies}-${config.maxZombies})`);
        assert(config.spawnStick ? sticks.length > 0 : sticks.length === 0, `Level ${level}: stick ${config.spawnStick ? 'spawned' : 'not spawned'}`);
        assert(config.spawnShovel ? shovels.length > 0 : shovels.length === 0, `Level ${level}: shovel ${config.spawnShovel ? 'spawned' : 'not spawned'}`);
        ...
        assert(player.position.x === 10 && player.position.y === 10, `Level ${level}: player at (10,10)`);
    }
}
...
async function testHoleDeath() {
    ...
    const afterState = mockServer.getState();
    assert(afterState.gamePhase === 'FINISHED' && afterState.endReason === 'fell_in_hole', 'Player died from falling in hole');
}
async function testZombieCatch() {
    ...
    const afterState = mockServer.getState();
    assert(afterState.gamePhase === 'FINISHED' && afterState.endReason === 'zombie_catch', 'Player died from zombie catch');
}
async function testWinCondition() {
    ...
    assert(finalState.gamePhase === 'FINISHED' && finalState.endReason === 'victory', 'Game ended with victory');
}
